...
class NativeLandmarkArchive(nn.Module):
    ...
    def __init__(self, d_model: int, landmark_dim: int = 128,
        ...
        # Cache de embeddings procesados (invalida cuando hay nuevo landmark)
        self._lm_cache     = None   # [n, landmark_dim] procesado con self-attn
        self._lm_cache_n   = -1
    ...
    def maybe_archive(self,
        ...
        # ── Threshold adaptativo EMA ─────────────────────────────────────────
        mean_err  = ttt_importance.mean().item() if ttt_importance is not None else 0.0
        full_prob = tier_probs[:, 2].mean().item()

        # Actualizar EMA (solo cuando hay señal real)
        if ttt_importance is not None:
            a     = self._ema_alpha
            mean  = self._err_ema_mean.item()
            var   = self._err_ema_var.item()
            delta = mean_err - mean
            self._err_ema_mean.fill_(mean + a * delta)
            self._err_ema_var.fill_((1 - a) * (var + a * delta ** 2))

        adaptive_threshold = (self._err_ema_mean.item() +
                              0.5 * math.sqrt(max(self._err_ema_var.item(), 1e-8)))
        # Fallback al umbral fijo si el EMA aún no ha convergido (pocos pasos)
        threshold = max(adaptive_threshold, self.ttt_err_threshold)

        if mean_err < threshold and full_prob < 0.5:
            return False
    ...
    def _store_landmark(self, emb: torch.Tensor, importance: float):
        ...
        # Invalida la caché de embeddings procesados
        self._lm_cache   = None
        self._lm_cache_n = -1
    ...
    def _get_processed_landmarks(self, device):
        ...
        Cacheado — no se recomputa hasta nuevo archivado.
        """
        n = self.n_archived.item()
        if n == 0:
            return None
        ...
        # Cache hit
        if self._lm_cache is not None and self._lm_cache_n == n:
            return self._lm_cache.to(device, dtype=self.lm_q1.weight.dtype)

        lm = self.archived_embeddings[:n].to(device, dtype=self.lm_q1.weight.dtype)  # [n, landmark_dim]
        ...
        self._lm_cache   = combined.detach()
        self._lm_cache_n = n
        return combined
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # COMPILED PATH: graph-break-free retrieve
    # ─────────────────────────────────────────────────────────────────────────

    @torch.no_grad()
    def precompute_retrieve_cache(self):
        """
        Pre-computa KV para retrieve sin graph breaks. Llama el trainer
        ANTES de cada forward compilado (o después de archive_deferred).

        Almacena en self._compiled_cache un dict con los tensores de KV
        pre-proyectados. Si el archive está vacío, _compiled_cache = None.

        Coste: O(n_lm × landmark_dim²) — negligible para n_lm ≤ 64.
        """
        n = self.n_archived.item()
        if n == 0:
        ...
        device = self.archived_embeddings.device
        dtype = self.ret_k1.weight.dtype

        # Obtener landmarks procesados (con self-attention)
        lm = self._get_processed_landmarks(device)
        if lm is None:
            self._compiled_cache = None
            return

        # Pre-proyectar K1, K2, V para retrieve
        K1 = self.ret_k1(lm)   # [n_lm, lm_dim]
        K2 = self.ret_k2(lm)
        V  = self.ret_v(lm)    # [n_lm, lm_dim]
        lam = torch.sigmoid(self.ret_lam)  # tensor, no .item()

        self._compiled_cache = {
            'K1': K1.detach(), 'K2': K2.detach(), 'V': V.detach(),
            'lam': lam.detach(), 'n_lm': lm.shape[0],
        }

    def retrieve_compiled(self, scan_out: torch.Tensor) -> torch.Tensor:
        """
        Retrieve graph-break-free para uso con torch.compile.

        Usa _compiled_cache pre-computado (sin .item(), sin _get_processed_landmarks).
        Si cache es None (archive vacío), retorna scan_out sin modificar.
        """
        ...
        q_repr = scan_out.mean(dim=1).detach()   # [B, D]
        Q1 = self.ret_q1(q_repr)                 # [B, lm_dim]
        Q2 = self.ret_q2(q_repr)

        K1, K2, V = cache['K1'], cache['K2'], cache['V']
        lam_val = cache['lam']

        # Diff-attn inline sin .item() — PyTorch path (compile-friendly)
        scale = 1.0 / math.sqrt(Q1.shape[-1])
        s1 = (Q1.float() @ K1.float().T) * scale       # [B, n_lm]
        s2 = (Q2.float() @ K2.float().T) * scale
        a1 = torch.softmax(s1, dim=-1)
        a2 = torch.softmax(s2, dim=-1)
        ret_emb = (a1 @ V.float() - lam_val * (a2 @ V.float())).to(scan_out.dtype)

        ret_emb = self.ret_norm(ret_emb)             # [B, lm_dim]
        ctx = self.expand(ret_emb).unsqueeze(1).expand(B, S, D)
    ...
    def get_archive_info(self):
        ...
        return {
            ...
            'cache_valid':      self._lm_cache is not None,
...
if __name__ == "__main__":
    ...
    print(f"[OK] cache válido: {archive._lm_cache is not None}")
